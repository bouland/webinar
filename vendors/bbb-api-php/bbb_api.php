...
class BigBlueButton {
	...
	*/
	public function elgg_createMeetingArray(ElggUser $user) {
		return $this->createMeetingArray( $user->name, $this->meetingID, $this->welcomeString, $this->modPW, $this->attPW, $this->securitySalt, $this->URL, $this->logoutURL );
	}
	...
	*/
	public function getMeetings() {
		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::getMeetingsURL( $this->URL, $this->securitySalt ) );
		if( $xml && $xml->returncode == 'SUCCESS' ) {
			if( $xml->messageKey )
			...
			if( count( $xml->meetings ) && count( $xml->meetings->meeting ) ) {
				...
				{
					echo '<meeting>';
					echo BigBlueButton::getMeetingInfo($meeting->meetingID, $meeting->moderatorPW, $this->URL, $this->securitySalt);
					echo '</meeting>';
				}
			}
			echo '</meetings>';
	...
	*/
	public function elgg_endMeeting(){
		return $this->endMeeting($this->meetingID, $this->modPW, $this->URL, $this->securitySalt);
	}
	...
	*/
	public function isMeetingRunning( $meetingID ) {
		$xml = bbb_wrap_simplexml_load_file( BigBlueButton::isMeetingRunningURL( $meetingID, $this->URL, $this->securitySalt ) );
		if( $xml && $xml->returncode == 'SUCCESS' ) 
